#!/usr/bin/env python3
"""Server-side Odoo backup and active plain FTP delivery; no desktop dependency."""
import argparse
import datetime
import fcntl
import ftplib
...
CONFIG = Path('/etc/odoo-backup-ftp.json')
...
def upload_run(settings, run):
    ...
    # Plain FTP and active mode explicitly requested by the user.
    with ftplib.FTP(timeout=120) as ftp:
        ...
        receipt_path = run / 'ftp-receipt.json'
        ...
        save_json(receipt_path, receipt)
        (run / '.ftp-pending').unlink(missing_ok=True)
...
def main():
    ...
    with (ROOT / '.ftp.lock').open('w') as lock:
        ...
        (run / '.ftp-pending').touch()
        failures = []
        for marker in sorted(ROOT.glob('*/.ftp-pending')):
            try:
