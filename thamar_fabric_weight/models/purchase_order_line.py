...
class PurchaseOrderLine(models.Model):
    ...
    def _compute_fabric_pieces(self):
        for line in self:
            if not line.fabric_pieces_manual:
                if line.is_fabric and line.product_qty and line.product_id.fabric_approx_weight:
                    line.fabric_pieces = round(line.product_qty / line.product_id.fabric_approx_weight, 2)
                else:
    ...
    @api.depends('product_qty', 'fabric_pieces')
    def _compute_fabric_avg_weight(self):
        for line in self:
            if line.is_fabric and line.fabric_pieces:
                line.fabric_avg_weight = line.product_qty / line.fabric_pieces
            else:
