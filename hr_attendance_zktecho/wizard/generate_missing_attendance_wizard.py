...
class generate_missing_attendance(models.TransientModel):
    ...
    def get_day_hours(self, employee, day_id ,date, action):
        ...
        days = calendar.attendance_ids.filtered(lambda d:d.dayofweek == str(day_of_week[day_id])).sorted(lambda line: line.hour_from)
        if days:
            if action == 'in' and days[0].hour_from < date.hour:
                time_hour = days[0].hour_from
            elif action == 'out' and days[-1].hour_to > date.hour:
                time_hour = days[-1].hour_to
        return time_hour
